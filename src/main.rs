...
fn double_file_extension(source_path: &str) -> String {
    ...
    format!("{}.{}{}", stem.to_str().unwrap(), extension.to_str().unwrap(), extension.to_str().unwrap())
}

fn main() {
    let source_path = "a.xlsx";
    let target_path = double_file_extension(source_path);

    if let Err(error) = read_bytes(source_path).and_then(|content| save_bytes(&content, &target_path)) {
        println!("Error: {}", error);
    } else {
